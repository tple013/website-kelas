        ...
        <div className="space-y-0">
          {sortedItems.map((item, index) => (
            <div key={item.id} className="relative pl-8 pb-8 last:pb-0">
              ...
              {/* Dot */}
              <div className="absolute left-0 top-1.5 h-6 w-6 rounded-full border-4 border-white bg-slate-300 group-hover/card:bg-blue-600 transition-colors shadow-sm z-10"></div>

              {/* Content */}
              <div className="relative -top-1">
                <span className="inline-block bg-blue-50 text-blue-700 text-[11px] font-bold px-2.5 py-1 rounded-md border border-blue-100 mb-2">
                  {item.time}
                </span>
                <h3 className="text-lg font-bold text-slate-800 leading-tight mb-2 group-hover/card:text-blue-700 transition-colors">
                  {item.subject}
                </h3>
                <div className="space-y-2">
                  ...
                  {item.instructor && (
                    <div className="flex items-start gap-2.5 text-sm text-slate-500">
                      <i className="bi bi-person-workspace text-slate-400 mt-0.5"></i>
                      <span className="text-slate-600 font-medium">{item.instructor}</span>
                    </div>
                  )}
                </div>
              </div>
