    ...
    <footer className="bg-slate-900 text-slate-300 border-t border-slate-800 mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8">
          ...
          {/* KOLOM 2: LINK CEPAT */}
          <div>
            <h4 className="text-white font-semibold text-lg mb-6 border-b border-blue-500/30 pb-2 inline-block">
              Navigasi
            </h4>
            <ul className="space-y-3">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  ...
                  >
                    <i className={`bi ${link.icon} text-slate-500 group-hover:text-sky-400 transition-colors`}></i>
            ...
            <ul className="space-y-4">
              {contactInfo.map((info, index) => (
                <li key={index} className="flex items-start space-x-3">
                  <i className={`bi ${info.icon} text-blue-500 mt-1`}></i>
